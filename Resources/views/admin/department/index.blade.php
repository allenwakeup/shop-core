        ...
        <div class="layui-card-body">
            <table class="layui-table" lay-filter="table" lay-data="{url:'{{ route('admin::' . module_route_prefix ('.') . 'core.department.list') }}?{{ request()->getQueryString() }}', page:true, limit:50, id:'test', toolbar:'<div><a href=\'{{ route('admin::' . module_route_prefix ('.') . 'core.department.create') }}\'><i class=\'layui-icon layui-icon-add-1\'></i>新增部门</a></div>'}" lay-filter="test">
                ...
                <tr>
                    ...
                    <th lay-data="{templet:'#departmentName'}">名称</th>
                    @include('admin.listHead', ['data' => Goodcatch\Modules\Core\Model\Admin\Department::$listField])
...
<script type="text/html" id="action">
    ...
    <a href="javascript:;" class="layui-table-link" title="删除" style="margin-left: 10px" onclick="deleteDepartment ('<% d.deleteUrl %>')"><i class="layui-icon layui-icon-delete"></i></a>
</script>
<script type="text/html" id="departmentName">
    <a href="?pid=<% d.id %>" class="layui-table-link"><% d.name %></a>
</script>
@include('core::admin.listHeadTpl', ['data' => Goodcatch\Modules\Core\Model\Admin\Department::$listField])
@section('js')
    <script>
